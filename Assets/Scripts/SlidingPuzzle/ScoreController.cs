...
namespace SlidingPuzzle
{
    ...
    public class ScoreController : MonoBehaviour
    {
       ...
       private TextMeshProUGUI winScreenHighScoreText, winScreenYourScoreText, failScreenYourScoreText;
        ...
        public int HighScore
        {
            get
            {
                return PlayerPrefs.GetInt("HighScorePuzzle", 0);
            }
            set
            {
                int temp = HighScore;
                if (value > temp)
                {
                    PlayerPrefs.SetInt("HighScorePuzzle", value);
                }
            }
        }
        public int LastScore
        {
            get
            {
                return PlayerPrefs.GetInt("LastScorePuzzle", 0);
            }
            set
            {
                PlayerPrefs.SetInt("LastScorePuzzle", value);
            }
        }
        ...
        */
        ...
        {
            float remainingTime = GameManager.Instance.TimeController.TimeRemaining;

            Score = (int)(remainingTime * 10);
            Score -= GameManager.Instance.CurrentMovement;
            Score += GameManager.Instance.PieceCarryRemainder * 50;
            Score += GameManager.Instance.HintRemainder * 10;
            if (Score<0)
                Score = 0;
